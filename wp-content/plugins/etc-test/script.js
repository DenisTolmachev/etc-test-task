jQuery(document).ready(function ($) {
  ...
  $("#search").click(function (e) {
    ...
    if ($("#build_name").val().length !== 0) {
      var name_house = $("#build_name").val();
    }
    ...
    $.ajax({
        ...
        if (response) {
          for (var i = 0; i < response.length; i++) {
            for (var j = 0; j < response[i].compared.length; j++) {
              var htm =
                "<li id='real-estate-" + response[i].compared[j].id + "'>";
              htm +=
                "  <a href='" +
              ...
              htm +=
                "          <img class='etc-item-img' src='" + response[i].compared[j].image + "' />";
              htm +=
                "          <h2>Приміщення " + response[i].name_house + "</h2>";
              htm +=
                "          <span class='cat-name'>" +
                response[i].tax_name +
                "</span>";
              ...
              htm +=
                "          <p>К-сть кімнат: " +
                response[i].compared[j].rooms +
                "</p>";
              htm +=
                "           <p>Площа: " +
                response[i].compared[j].square +
                "</p>";
              htm +=
                "           <p>Балкон: <span>" +
                response[i].compared[j].balcony["label"] +
                "</span></p>";
              htm +=
                "          <p>Санвузол: " +
                response[i].compared[j].bedroom["label"] +
                "</p>";
              htm += "      </div>";
              htm += "      </div>";
              htm += "  </a>";
            ...
            html += "          <img class='etc-item-img' src='" + response[i].image + "' /> ";
            ...
            html +=
              ...
              "</p>";
            html +=
              "          <p>Координати: " + response[i].coordinate + "</p>";
      ...
      error: function (xhr, errorThrown) {
        mainBox.empty();
        var html =
          "<p class='error'>Жодних співпадінь не знайдено, спробуйте інший запит.</p>";
        mainBox.append(html);
      },
    });
  });
